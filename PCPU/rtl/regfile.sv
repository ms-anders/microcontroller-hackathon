...
module regfile (
    ...
    always @(posedge clk) begin
        if (wr_en && wr_addr != 30)  // 30 = zero register
    ...
    assign rd_data_a = (rd_addr_a == 30) ? {`DATA_WIDTH{1'b0}} : regs[rd_addr_a];
    assign rd_data_b = (rd_addr_b == 30) ? {`DATA_WIDTH{1'b0}} : regs[rd_addr_b];
